...
import { type HsvaColor, random } from "colord";

export default function ColorInfo(props: { hex: string; hsl: string; action: Dispatch<SetStateAction<HsvaColor>> }) {
  const colorHEX = props.hex;
  const colorHSL = props.hsl;
  const randomColor = () => props.action(random().toHsv());

  return (
    <div className="color-info" role="none">
      <pre className="color-metadata">
